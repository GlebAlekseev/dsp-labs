...
data class LPFAmplitudeData(
    private val axis: List<Pair<List<Double>, List<Double>>>, override val hasLegend: Boolean = true
) : Data {
    override val axisX = listOf(
        "АЧХ" to mutableListOf<Double>(),
        "Требуемые характеристики ФНЧ" to mutableListOf<Double>(),
        "Верхняя граница полосы пропускания" to mutableListOf<Double>(),
        "Нижняя граница полосы пропускания" to mutableListOf<Double>(),
        "Верхняя граница полосы задерживания" to mutableListOf<Double>(),
    )
