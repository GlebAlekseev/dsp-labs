...
class SAPSRepository {
    fun getAveragePower() = (2 * VARIABLE_EMax * VARIABLE_tu) / VARIABLE_T

    private fun getA0Factor() = 0

    @Throws(DividerException::class)
    private fun getAnFactor(n: Int): Double {
        ...
        if (divider == 0.0) throw DividerException()
        return numerator / divider
    }

    @Throws(DividerException::class)
    private fun getBnFactor(n: Int): Double {
    ...
    fun getPhase(n: Int): Double {
        ...
        if (anFactor == 0.0) throw DividerException()
        return atan(getBnFactor(n) / anFactor)
    }
    ...
    fun getMaxNumberOfHarmonics(delta: Double): Int {
        ...
        while (true) {
            ...
            val pk = left + 1 / 2 * right
            if ((pc - pk) / pk > delta) return n - 1
            n++
        }
    }
